      ...
      home: FutureBuilder<bool>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return PageA(title: ''); // Navigate to Page2 if connected
          } else {
            return SignInPage(); // Navigate to SignInPage if not connected
          }
        },
      ),
    );
    ...
    return FutureBuilder<String?>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (token != null) {
            return Page2(); // Navigate to Page2 if token is present
          } else {
            return SignInPage(); // Navigate to SignInPage if token is null
          }
        } else {
          return Scaffold(
            backgroundColor: Colors.black,
